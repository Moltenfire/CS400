...
THEORY ListVariablesX IS
  ...
  Local_List_Variables(Refinement(Buffer_r))==(num,pos,aa);
  List_Variables(Refinement(Buffer_r))==(num,pos,aa);
  External_List_Variables(Refinement(Buffer_r))==(num,pos,aa)
END
&
...
THEORY ListOfNewVariablesX IS
  List_Of_New_Variables(Refinement(Buffer_r))==(num,pos,aa)
END
&
THEORY ListInvariantX IS
  ...
  List_Invariant(Refinement(Buffer_r))==(aa: 1..qlen --> ELEM & pos: NAT1 & pos<=qlen & num: NAT & num<=qlen & num = size(buff) & aa\|/pos-1^(aa/|\pos-1)/|\num = buff)
END
&
...
THEORY ListInitialisationX IS
  Expanded_List_Initialisation(Refinement(Buffer_r))==(@(aa$0).(aa$0: 1..qlen --> ELEM ==> aa:=aa$0);pos:=1;num:=0);
  Context_List_Initialisation(Refinement(Buffer_r))==(skip);
  List_Initialisation(Refinement(Buffer_r))==(aa:: 1..qlen --> ELEM;pos:=1;num:=0)
END
&
...
THEORY ListSubstitutionX IS
  Expanded_List_Substitution(Refinement(Buffer_r),contentsquery)==(btrue | oo:=num);
  Expanded_List_Substitution(Refinement(Buffer_r),fullquery)==(btrue | num = qlen ==> rr:=yes [] not(num = qlen) ==> rr:=no);
  Expanded_List_Substitution(Refinement(Buffer_r),emptyquery)==(btrue | num = 0 ==> rr:=yes [] not(num = 0) ==> rr:=no);
  Expanded_List_Substitution(Refinement(Buffer_r),remove)==(buff/=<> | num>0 ==> (ee:=aa(pos);num:=num-1;pos:=pos mod qlen+1) [] not(num>0) ==> skip);
  Expanded_List_Substitution(Refinement(Buffer_r),add)==(xx: ELEM & size(buff)<qlen | num>qlen ==> (aa:=aa<+{(pos+num-1) mod qlen+1|->xx};num:=num+1) [] not(num>qlen) ==> skip);
  List_Substitution(Refinement(Buffer_r),add)==(IF num>qlen THEN aa((pos+num-1) mod qlen+1):=xx;num:=num+1 END);
  List_Substitution(Refinement(Buffer_r),remove)==(IF num>0 THEN ee:=aa(pos);num:=num-1;pos:=pos mod qlen+1 END);
  List_Substitution(Refinement(Buffer_r),emptyquery)==(IF num = 0 THEN rr:=yes ELSE rr:=no END);
  List_Substitution(Refinement(Buffer_r),fullquery)==(IF num = qlen THEN rr:=yes ELSE rr:=no END);
  List_Substitution(Refinement(Buffer_r),contentsquery)==(oo:=num)
END
&
...
THEORY ListANYVarX IS
  ...
  List_ANY_Var(Refinement(Buffer_r),?)==(?)
END
&
THEORY ListOfIdsX IS
  List_Of_Ids(Refinement(Buffer_r)) == (? | ? | num,pos,aa | ? | add,remove,emptyquery,fullquery,contentsquery | ? | ? | qlen,ELEM | Buffer_r);
...
THEORY VariablesEnvX IS
  Variables(Refinement(Buffer_r)) == (Type(num) == Mvl(btype(INTEGER,?,?));Type(pos) == Mvl(btype(INTEGER,?,?));Type(aa) == Mvl(SetOf(btype(INTEGER,1,qlen)*atype(ELEM,?,?))))
END
&
